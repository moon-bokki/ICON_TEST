...
- **파일 내려받기** — 라인 아이콘 상세 패널의 **파일 내려받기** 항목에서 **SVG** 와 **PNG** 로
  저장할 수 있습니다. SVG 는 벡터라 크기와 무관하며 툴바에서 고른 두께·색이 그대로 담기고,
  PNG 는 지정한 픽셀(8~512px, @1x/@2x/@3x)로 실제 래스터 이미지를 구워 줍니다. 색이
  `currentColor` 인 SVG 는 쓰는 곳의 글자색을 따라가고, PNG 는 색을 상속할 수 없어 굽는
  시점의 글자색으로 고정됩니다.
  cafe On 이미지 아이콘에는 **원본 파일** 버튼이 있어 변환하지 않은 원본을 그대로 받을 수
  있습니다. 크기를 바꿔 저장하려면 기존의 **크기 변환 · 내보내기**를 쓰세요.
...
- **분류 버튼 추가** — 툴바의 **+ 분류 추가** 버튼으로 빈 분류를 미리 만들어 둘 수 있습니다.
  원래 분류는 "아이콘이 가리키는 값"이라 소속 아이콘이 0개면 존재할 수 없는데, 만들어 둔
  사실을 따로 기록해 **아이콘이 없어도 칩이 남습니다.** 상세 패널의 분류 드롭다운에도 바로
  나타나므로 아이콘을 옮겨 담기 좋습니다. 이미 있는 이름과 `전체`·`cafe On`·`숨김` 은
  입력 단계에서 막습니다.
- **분류 버튼 삭제 · 되살리기** — 상단 분류 칩에 마우스를 올리면 오른쪽 위에 **×** 가 나타납니다.
  누르면 그 분류 칩이 필터 목록에서 사라집니다. **아이콘은 전혀 건드리지 않으므로** 소속
  아이콘은 `전체` 와 검색에 그대로 남고, 상세 패널의 분류 표시도 그대로입니다.
  하나라도 지우면 **분류 되살리기** 칩이 나타나 한 번에 되돌릴 수 있습니다.
  `기타` 는 기본 분류라 지울 수 없습니다.
