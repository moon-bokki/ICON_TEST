...
/** 응답 헤더가 없을 때 확장자로 MIME 타입을 추정한다 */
const MIME_BY_EXT = {
  svg: 'image/svg+xml',
  gif: 'image/gif',
  png: 'image/png',
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!alive) return;
        // 타입을 빠뜨리면 Content-Type 이 빈 문자열이 되고, 그러면 <img> 가 SVG 를
        // 그리지 않는다 (GIF·PNG 는 매직 바이트로 판별하지만 SVG 는 명시가 필수).
        const blob = new Blob([buf], { type: res.headers.get('content-type') || guessType(src) });
